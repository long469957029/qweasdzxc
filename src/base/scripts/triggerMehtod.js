Base._triggerMethod = (function() {
  ...
  return function(context, event, args) {
    const noEventArg = arguments.length < 3
    if (noEventArg) {
      args = event
      event = args[0]
    }
    ...
    if (_.isFunction(context.trigger)) {
      if (noEventArg + args.length > 1) {
        context.trigger(...noEventArg ? args : [event].concat(_.rest(args, 0)))
      } else {
...
Base.triggerMethodOn = function(context) {
  const fnc = _.isFunction(context.triggerMethod) ? context.triggerMethod : Base.triggerMethod
