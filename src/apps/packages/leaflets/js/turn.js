...
(function($) {
  var has3d,
    ...
    turnMethods = {
      ...
      _turnPage(page) {
        ...
        let optsCorners = data.opts.turnCorners.split(','),
        ...
        switch (data.effect) {
          ...
          `translate3d(0px, 0px, ${this.attr('depth') || 0}px)`, parentOrigin)
            ...
            backgroundColor: 'rgba(0,0,0,'+(0.1)+')'
              ...
              gradient(
                data.bshadow, point2D(gradientX * 100, 0), point2D((-gradientX + 1) * 100, 0),
  ...
  function dec(that, methods, args) {
    if (!args[0] || typeof (args[0]) === 'object') { return methods.init.apply(that, args) } else if (methods[args[0]]) { return methods[args[0]].apply(that, Array.prototype.slice.call(args, 1)) }
  ...
  function trigger(eventName, context, args) {
    ...
    if (event.isDefaultPrevented()) { return 'prevented' } else if (event.isPropagationStopped()) { return 'stopped' }
    return ''
  }
