...
(function(global, factory) {
  ...
  const SLIMSCROLL_BAR_SEL = '.slimScrollBar'
  const SLIMSCROLL_RAIL_SEL = '.slimScrollRail'
  ...
  const SECTION_FIRST_SEL = `${SECTION_SEL}:first`
  const SECTION_LAST_SEL = `${SECTION_SEL}:last`
  const TABLE_CELL = 'fp-tableCell'
  const TABLE_CELL_SEL = `.${TABLE_CELL}`
  const AUTO_HEIGHT = 'fp-auto-height'
  const AUTO_HEIGHT_SEL = '.fp-auto-height'
  ...
  $.fn.fullpage = function(options) {
    ...
    FP.setAutoScrolling = function(value, type) {
      ...
      const element = $(SECTION_ACTIVE_SEL)
    ...
    FP.reBuild = function(resizing) {
      ...
      $(SECTION_SEL).each(function() {
        const slidesWrap = $(this).find(SLIDES_WRAPPER_SEL)
        const slides = $(this).find(SLIDE_SEL)
      ...
      const activeSection = $(SECTION_ACTIVE_SEL)
      const sectionIndex = activeSection.index(SECTION_SEL)
    ...
    function init() {
      ...
      const activeSlide = $(SECTION_ACTIVE_SEL).find(SLIDE_ACTIVE_SEL)
    ...
    function prepareDom() {
      ...
      $(SECTION_SEL).each(function(index) {
        const section = $(this)
        const slides = section.find(SLIDE_SEL)
    ...
    function styleSlides(section, slides, numSlides) {
      ...
      const startingSlide = section.find(SLIDE_ACTIVE_SEL)
    ...
    function addVerticalNavigation() {
      ...
      $(SECTION_NAV_SEL).find('li').eq($(SECTION_ACTIVE_SEL).index(SECTION_SEL)).find('a')
        .addClass(ACTIVE)
    }
    ...
    function afterRenderActions() {
      const section = $(SECTION_ACTIVE_SEL)
    ...
    function solveBugSlimScroll(section) {
      const slides = section.find('SLIDES_WRAPPER')
      let scrollableWrap = section.find(SCROLLABLE_SEL)
    ...
    function scrollHandler() {
      ...
      if (!options.autoScrolling || options.scrollBar) {
        ...
        let initial = Math.abs(currentScroll - document.querySelectorAll(SECTION_SEL)[0].offsetTop)
        ...
        for (let i = 0; i < sections.length; ++i) {
          ...
          const current = Math.abs(currentScroll - section.offsetTop)
        ...
        if (!currentSection.hasClass(ACTIVE) && !currentSection.hasClass(AUTO_HEIGHT)) {
          isScrolling = true
          const leavingSection = $(SECTION_ACTIVE_SEL)
          const leavingSectionIndex = leavingSection.index(SECTION_SEL) + 1
          const yMovement = getYmovement(currentSection)
          const anchorLink = currentSection.data('anchor')
          const sectionIndex = currentSection.index(SECTION_SEL) + 1
          const activeSlide = currentSection.find(SLIDE_ACTIVE_SEL)
    ...
    function touchMoveHandler(event) {
      ...
      if (!checkParentForNormalScrollElement(event.target) && isReallyTouch(e)) {
        ...
        const activeSection = $(SECTION_ACTIVE_SEL)
        const scrollable = isScrollable(activeSection)
    ...
    function moveSlide(direction) {
      const activeSection = $(SECTION_ACTIVE_SEL)
      const slides = activeSection.find(SLIDES_WRAPPER_SEL)
      const numSlides = slides.find(SLIDE_SEL).length
      ...
      const currentSlide = slides.find(SLIDE_ACTIVE_SEL)
      let destiny = null
    ...
    function scrollPage(element, callback, isMovementUp) {
      ...
      requestAnimFrame(() => {
        const dest = element.position()
        if (typeof dest === 'undefined') { return } // there's no element to scroll, leaving the function

        // auto height? Scrolling only a bit, the next element's height. Otherwise the whole viewport.
        const dtop = element.hasClass(AUTO_HEIGHT) ? (dest.top - windowsHeight + element.height()) : dest.top
    ...
    function performMovement(v) {
      ...
      if (options.css3 && options.autoScrolling && !options.scrollBar) {
        const translate3d = `translate3d(0px, -${v.dtop}px, 0px)`
      ...
      // using jQuery animate
        ...
        const scrollSettings = getScrollSettings(v)

        $(scrollSettings.element).animate(
          scrollSettings.options,
          options.scrollingSpeed, options.easing,
        ).promise().done(() => { // only one single callback in case of animating  `html, body`
          afterSectionLoads(v)
        })
      }
    }
    ...
    function lazyLoad(destiny) {
      ...
      const slide = destiny.find(SLIDE_ACTIVE_SEL)
      if (slide.length) {
        destiny = $(slide)
      }
    ...
    function hashChangeHandler() {
      if (!isScrolling && !options.lockAnchors) {
        ...
        const isFirstScrollMove = (typeof lastScrolledDestiny === 'undefined' && typeof slide === 'undefined' && !slideMoving)
    ...
    $document.on('click touchstart', SLIDES_NAV_LINK_SEL, function(e) {
      e.preventDefault()
      const slides = $(this).closest(SECTION_SEL).find(SLIDES_WRAPPER_SEL)
      const destiny = slides.find(SLIDE_SEL).eq($(this).closest('li').index())
    ...
    function landscapeScroll(slides, destiny) {
      const destinyPos = destiny.position()
      const slideIndex = destiny.index()
      const section = slides.closest(SECTION_SEL)
      const sectionIndex = section.index(SECTION_SEL)
      const anchorLink = section.data('anchor')
      const slidesNav = section.find(SLIDES_NAV_SEL)
      const slideAnchor = getAnchor(destiny)

      // caching the value of isResizing at the momment the function is called
      // because it will be checked later inside a setTimeout and the value might change
      const localIsResizing = isResizing

      if (options.onSlideLeave) {
        const prevSlide = section.find(SLIDE_ACTIVE_SEL)
        const prevSlideIndex = prevSlide.index()
        const xMovement = getXmovement(prevSlideIndex, slideIndex)
      ...
      const afterSlideLoads = function() {
        // if the site is not just resizing and readjusting the slides
        if (!localIsResizing) {
    ...
    function responsive() {
      ...
      const isBreakingPointWidth = widthLimit && $window.width() < widthLimit
      const isBreakingPointHeight = heightLimit && $window.height() < heightLimit
    ...
    function resizeMe(displayHeight, displayWidth) {
      ...
      const preferredHeight = 825
      const preferredWidth = 900

      if (displayHeight < preferredHeight || displayWidth < preferredWidth) {
        const heightPercentage = (displayHeight * 100) / preferredHeight
        const widthPercentage = (displayWidth * 100) / preferredWidth
        const percentage = Math.min(heightPercentage, widthPercentage)
        const newFontSize = percentage.toFixed(2)
    ...
    function activateNavDots(name, sectionIndex) {
      if (options.navigation) {
        ...
        if (name) {
          ...
          $(SECTION_NAV_SEL).find('li').eq(sectionIndex).find('a')
            .addClass(ACTIVE)
        }
      }
    }
    ...
    function getYmovement(destiny) {
      const fromIndex = $(SECTION_ACTIVE_SEL).index(SECTION_SEL)
      const toIndex = destiny.index(SECTION_SEL)
    ...
    function createSlimScrolling(element) {
      ...
      const section = element.closest(SECTION_SEL)
      const scrollable = element.find(SCROLLABLE_SEL)
      let contentHeight
      ...
      const scrollHeight = windowsHeight - parseInt(section.css('padding-bottom')) - parseInt(section.css('padding-top'))
    ...
    function removeSlimScroll(element) {
      element.find(SCROLLABLE_SEL).children().first().unwrap()
        .unwrap()
    ...
    function getTableHeight(element) {
      ...
      if (options.paddingTop || options.paddingBottom) {
        ...
        const paddings = parseInt(section.css('padding-top')) + parseInt(section.css('padding-bottom'))
    ...
    function getSectionByAnchor(sectionAnchor) {
      // section
      let section = $(`${SECTION_SEL}[data-anchor="${sectionAnchor}"]`)
      if (!section.length) {
    ...
    function getSlideByAnchor(slideAnchor, section) {
      const slides = section.find(SLIDES_WRAPPER_SEL)
      let slide = slides.find(`${SLIDE_SEL}[data-anchor="${slideAnchor}"]`)
    ...
    function scrollPageAndSlide(destiny, slide) {
      const section = getSectionByAnchor(destiny)
    ...
    function scrollSlider(section, slideAnchor) {
      if (typeof slideAnchor !== 'undefined') {
        const slides = section.find(SLIDES_WRAPPER_SEL)
        const destiny = getSlideByAnchor(slideAnchor, section)
    ...
    function addSlidesNavigation(section, numSlides) {
      ...
      const nav = section.find(SLIDES_NAV_SEL)
    ...
    function setBodyClass() {
      const section = $(SECTION_ACTIVE_SEL)
      const slide = section.find(SLIDE_ACTIVE_SEL)
    ...
    function silentScroll(top) {
      if (options.scrollBar) {
        ...
        const translate3d = `translate3d(0px, -${top}px, 0px)`
    ...
    function destroyStructure() {
      ...
      $.each($body.get(0).className.split(/\s+/), (index, className) => {
