...
const springRechargeBetView = Base.ItemView.extend({
      ...
      .done((res) => {
        if (res && res.result === 0) {
          ...
          _(data.itemList).each((item, index) => {
            // var $span = self.$('.js-qe-spring-rb-ticket[data-id='+item.ticketId+']');
            // $span.html('返水'+_(item.amount).formatDiv(100)+'%');
            let imgSrcIndex = index + 1
            let rbHtml = ''
            if (imgSrcIndex <= 12) {
              imgSrcIndex = imgSrcIndex < 10 ? `0${imgSrcIndex}` : imgSrcIndex.toString()
              const imgSrc = require(`./images/cjct/animal_${imgSrcIndex}.png`)
              rbHtml += '<li class="js-qe-spring-rb-li">'
              rbHtml += `<img src="${imgSrc}" alt="" />`
              rbHtml += `<div class="game_01">${item.ticketName}<span class="js-qe-spring-rb-ticket" data-id="${item.ticketId}">返水${_(item.amount).formatDiv(100)}%</span></div>`
              rbHtml += '</li>'
              rbListHtml += rbHtml
            }
          })
          self.$('.js-qe-spring-rb').html(rbListHtml)
        }
      })
  },
  ...
  checkState ($target) {
    ...
    return { $dialog, dialogParent: '.js-qe-spring-recharge-bet', isShow }
  },
