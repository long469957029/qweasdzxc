...
const RecordView = Base.ItemView.extend({
  ...
  onRender() {
    ...
    $.when(this.GameXhr).done((res) => {
      if (res.result === 0) {
        self.$('.js-sideBar-Bet-Today').html(_(res.root.dataTotal.betTotal).convert2yuan().toFixed(2))// 总余额
        self.$('.js-sideBar-Profit-Today').html(_(res.root.dataTotal.profitTotal).convert2yuan().toFixed(2))// 可用余额
