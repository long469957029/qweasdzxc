...
const LowLevelManageView = SearchGrid.extend({
  ...
  formatRowData(rowInfo) {
    ...
    const freeze = (rowInfo.userStatus === 100 || rowInfo.userStatus === 101) ? '<span class="sfa sfa-freeze m-right-xs"></span>'
      : (rowInfo.userStatus === 103 ? '<span class="sfa sfa-recycling m-right-xs"></span>' : '')
  ...
  checkPayPwdSet (e) {
    ...
    if (Global.memoryCache.get('acctInfo').foundsLock) {
      Global.ui.notification.show('资金已锁定，暂不能进行转账操作')
      return false
    }
