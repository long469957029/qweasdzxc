...
const RedPacketView = SearchGrid.extend({
    ...
    this.getRuleXhr()
      .done((res) => {
        ...
        if (res && res.result === 0) {
          ...
          self.$('.js-ac-rewards').html(_(data.itemList).map((item) => {
  ...
  formatRowData(rowInfo) {
    ...
    row.push(`<a href="${_.addHrefArgs('#fc/ad', 'tradeNo', rowInfo.tradeNo)}" class="router btn btn-link no-padding">查看明细</a>`)
